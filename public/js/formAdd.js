window.onload = async ()=>{
    ...
    console.log('Formulario de venta vinculado!');
try {
...
const regExExt = /(.jpg|.png|.jpeg|.gif|.webp)$/i;

    $('images').addEventListener('change', function (e) {
        switch (true) {
            case !regExExt.exec(this.value):
                $('error-images').innerHTML = "Sólo se admiten archivos jpg, png, jpeg, gif, webp"
                break;
            case this.files.length > 3:
                $('error-images').innerHTML = "Puedes subir hasta 3 imágenes"
            default:
                cleanError('error-images', e);
